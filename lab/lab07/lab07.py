...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(assignment, score):
        nonlocal class_assignments
        class_assignments["nom"] = class_assignments.get("nom", 0) + score
        class_assignments["den"] = class_assignments.get("den", 0) + class_assignments[assignment]
        return class_assignments["nom"] / class_assignments["den"]
    return helper
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1 :
        return 1
    return num_trees(n-1) * (2*n - 3) * (2*n - 2) // (n * (n - 1))
