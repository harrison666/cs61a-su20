...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place = self.place
        transition = 0
        while place.entrance is not beehive:
            if place.bees and transition >= self.min_range and transition <= self.max_range:
                break
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        print("DEBUG: self.contained_ant ", self.contained_ant)
        if self.contained_ant:
            #print("DEBUG: self.contained_ant.action ", self.contained_ant.action(gamestate))
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        print("DEBUG: self.place.bees[:]", self.place.bees[:])
        for bee in self.place.bees[:]:
            bee.reduce_armor(self.damage)
        if self.contained_ant:
            self.contained_ant.action(gamestate)
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        print("DEBUG: true_queen", self.true_queen)
        if self.true_queen:
            ScubaThrower.action(self, gamestate)
            place = self.place
            while place.exit:
                if not place.exit.ant:
                    place = place.exit
                    continue
                if isinstance(place.exit.ant, ContainerAnt) and place.exit.ant.contained_ant and not place.exit.ant.contained_ant.buffed :
                    place.exit.ant.contained_ant.damage *= 2
                    place.exit.ant.contained_ant.buffed = True
                if not place.exit.ant.buffed:
                    place.exit.ant.damage *= 2
                    place.exit.ant.buffed = True
                place = place.exit
        else:
...
def apply_status(status, bee, length):
    ...
    affected_action = status(bee.action, bee)
