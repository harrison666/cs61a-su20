...
def balanced(m):
    ...
    if is_planet(m):
        return True
    print("DEBUG: left",length(left(m))*total_weight(end(left(m))))
    print("DEBUG: right",length(right(m))*total_weight(end(right(m))))
    ...
    def helper(t, lst):
        if is_leaf(t):
            ...
            for branch in branches(t):
                lst += helper(branch, [label(branch)])
        return lst
...
def quadratic(x, a, b, c):
    ...
    def quad(t):
        return a*t*t + b*t + c
    extreme1 = quad(lower_bound(x))
    extreme2 = quad(upper_bound(x))
    if a != 0 and -b/(2*a) < upper_bound(x) and -b/(2*a) > lower_bound(x):
        extreme3 = quad(-b/(2*a))
        return interval(min(extreme3, extreme1, extreme2), max(extreme3, extreme1, extreme2))
    else:
        return interval(min(extreme1, extreme2), max(extreme1, extreme2))
